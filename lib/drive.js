...
async function listChildren(folderIds) {
  ...
  const url = `${DRIVE_API}/files?q=${encodeURIComponent(q)}&pageSize=200&fields=${encodeURIComponent(
    "files(id,name,mimeType,modifiedTime,parents,videoMediaMetadata,thumbnailLink,hasThumbnail,webContentLink)",
  )}`;
  ...
  const data = files
    ...
    .map((f) => {
      ...
      return {
        ...
        videoUrl: f.webContentLink || `https://drive.google.com/uc?id=${f.id}&export=download`,
